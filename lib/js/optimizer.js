...
/**
 * Iframe 높이 맞춤
 * @param {Object} oIframeElement Iframe 오브젝트
 * @param {Int} Iframe Height 추가 높이값
 */
...
/**
 * Ajax 호출 (jqeury ajax 간략버전)
 * @param {String} sUrl URL
 * @param {Object} mParams 파라미터값 (http://api.jquery.com/jQuery.param/)
 * @param {Object} oCallBackFunctoin 콜백함수
 * @param {String} sProgressType 프로그레스바 타입
 * @param {Bool} bIsErrorCheck 에러 검사 여부
 */
 ...
 * 에러검사
 * @param {Mix} mData 데이터
 * @param {Bool} bIsAlterMsg alter 메시지창 여부
 * @return {Bool} true / false
 */
...
/**
 * 어드민 URI를 반환 합니다.
 * @param {String} sLocatoin 위치
 * @param {String} sModuleName 모듈 이름
 * @return {String} 어드민 URI
 */
...
/**
 * 애드온 URI를 반환 합니다.
 * @param {String} sAddonName 애드온 이름
 * @param {String} sAddonMethodName 애드온 메서드 이름
 * @return {String} 애드온 URI
 */
...
/**
 * 모듈 및 애드온을 실행하는 URI를 반환 합니다.
 * @param {String} sPosition admin / front
 * @param {String} sXName 실행시킬 모듈, 애드온 이름
 * @param {String} sMethodName 실행시킬 메서드명 (※ 모듈에서는 methodchange로 동작 합니다.)
 * @param {String} sContentType 컨텐츠 타입
 * @return {String} 실행 URI
 */
...
/**
 * class로 dispaly 핸들링 합니다.
 * @param {Mix} mElement 엘리먼트 이름 / 엘리먼트 객체
 * @param {Bool} bIsDisplay 출력 여부 
 */
...
var Validator = 
{
    ...
    /**
     * 엘리먼트 한개만 검사 합니다.
     * @param {String} mElementId 엘리먼트 아이디, 여러개 검사시 배열로 입력
     * @param {Array} sFilterMessage 필터 메시지 ※ex - [{filter:'isFill',msg:'Message1'},{filter:'isNumber',msg:'Message2'}]
     * @return {Object} 검사 결과
     */
...
/**
 * input 태그에 속성값을 할당 합니다.
 */
var SetInput = 
{
    /**
     * 실행 메서드
     * @param {Object} oInputInfo Set input json 데이터값
     */
        ...
        try {
            ...
            switch(sType) {            
                ...
                case 'hidden':                                                 
                    ...
                    if(!oForm.attr('name')) {
                        alert('Hidden 타입은 Form 타입이 선언 되어야 합니다.');
                        return false;
                    }
    ...
    /**
     * 복수 체크박스 태그를 셀렉트(MULTIPLE) 태그로 변환
     */
    ...
    _setAttributeEvent : function(oElement, mVal)
    {
        for(sEvent in mVal) {
            if(sEvent == '0') {
                alert(oElement.attr('name')+'에 Event 속성값이 잘 못 할당 되어 있습니다.');
                return false;
            }
            if(isset(mVal[sEvent]) === true) {
                ...
                try {
                    ...
                    alert(oElement.attr('name')+'에 '+sEventVal+' 올바르지 않는 Event 속성값 입니다.');
                    return false;
                } 
            }
        }        
    ...
    _setAttributeExtra : function(oElement, mVal)
    {
        for(sExtra in mVal) {
            if(sExtra == '0') {
                alert(oElement.attr('name')+'에 Extra 속성값이 잘 못 할당 되어 있습니다.');
                return false;
            }
 ...
 * 
 * 아래는 반드시 필요함
 * init 에 전달되는 부모엘리먼트의 경우 display가 되어 있는 상태에서 초기화가 
 * 진행되어야 합니다. 그렇지 않을 경우 실제 엘리먼트 사이즈를 알수 없어 진행상태바가
 * 나오는 위치가 정상적이지 않게 됩니다.
 *  <code>
    $.Progress.init({
        'parents' : 진행바가 나타날 엘리먼트,
        'skin' : 사용 스킨(builder, ajax),
        'background' : BG 사용유무(true, false),
        'title' : builder skin 사용시 progress 창 title
    });
 *  </code>
 * 
 * 처리전
 ...
 * 
 * 
 * AJAX 처리시 show, hide 메소드를 활용해 상황에 맞게 노출시키도록 한다.
 * init메소드 내부에서 실제로 진행바엘리먼트를 생성하기 때문에 부모 엘리먼트의
 * 내용을 비운다거나 하는 이벤트가 발생시에는 다시 한번 init메소드를 발생시켜서
 * 진행바 엘리먼트를 생성시켜주어야 함을 유의한다.
 */
...
$.BfwValidator = {
    /**
     * 일반적인 submit 유효성 검사
     * @var flag : 폼전송:true, 사용자정의 값:false
     */
    ...
    /**
     * 초기화 모든 폼을 벨리데이션 이벤트를 준다.(submit);
     */
    init : function()
    {
        ...
        this.bIsFront       = false;
        this.eventFlag      = true;
        this.sEditWarnningCss   = 'border:2px solid #DC4E22;width:50%;';  //에디터 객체 에러 발생시 css
        this.bErrorFalg     = true;                 //true: 전체 에러객체, false: 첫번째에러 객체
        this.passed         = '';                   //validation 성공, 실패 Flag
        this.validator      = '';                   //validation 객체
        this.element        = '';                   //에러가 발생된 객체
        this.elmid          = '';                   //에러가 발생된 엘리먼트 아이디
        this.msg            = '';                   //에러메세지
        this.form           = '';                   //폼
        this.formid         = '';                   //폼아이디
        this.callback       = null;
        this.prevElement    = '';                   //이전 에러 객체
        this.bBlock         = false;
        this.bHasMessage    = false;
        
        this.defaultMessageType    = 'message';        //메세지를 보여줄 타입 'message:메세지, layout:레이아웃, window:새창'
        this.messageArea           = 'validation_message';   //메세지영역 id
        this.messageClass          = 'msg_warn_box';
        this.noticeClass           = 'msg_suc_box';
        this.subClass              = 'cle_l';
        
        this.messagePosition       = 'next';        //메세지영역이 없을 때 메세지 보여줄곳, (top:최상단, next:에러발생한 엘리먼트 다음)
        this.removeClass           = '';
        this.warningClass          = 'warn_border';
        this.frontWarningClass     = 'intimation_ver error';
        
        this.windowType         = 'dialog';         //팝업창 타입
        //this.windowUrl          = '/admin/error/';  //팝업창 url
        this.windowUrl          = '';
        this.windowTitle        = '';               //팝업창 title
        //팝업창 속성
        this.windowAttribute    = 'dialogTop=300px;dialogHeight=100px;dialogWidth=300px'; 
        
        //this.layoutUrl          = 'src="/admin/error/?msg=메세지보낼꺼야"';
        this.layoutUrl          = '';       //레이아웃 url
        //레이아웃 속성
...
//            alert($('[id^=auto_validator_message_]').size());
    ...
    //메세지 타입 설정(message, window, layout)
...
/**
 * submit을 통하지 않고 버튼으로 validator검사할 경우 폼이 존재할때
 * @var target : this값 (onclick=oValidator.buttonBeginForm.getMessage(this))
 */
...
/**
 * 사용자 메세지 출력(범용적인 곳에서 사용, 사용자가 메세지 호출하고싶을때 (플래그, 메세지) )
 */
...
/**
 * submit을 통하지 않고 버튼으로 validator검사할 경우
 * 폼이 존재하지않을때 
 * @var aTargets : name 이나 id값 배열['kwak','tae','won']
 */
$.BfwValidator.button={
...
/**
 *  에러 핸들러 상속 에러가 발생한 객체의 정보를 받아온다.
 */
...
$L.regist("ip_restriction", {
    "txt_ip_error" : "잘못된 형식입니다. 숫자로 입력해 주시기 바랍니다.",
    "txt_ip_existed_error" : "동일한 IP주소가 있습니다.",
    "txt_ip_incorrect_error" : "IP범위 차단 값이 올바르게 입력되지 않았습니다.",
    "txt_ip_restricted" : "은(는) 성공적으로 차단되었습니다.",
    "txt_ip_remove" : "삭제되었습니다."
});
$L.regist("newsletter", {
    "error_check" : "You have to check at least one item",
    "error_not_in" : "뉴스레터가 없습니다.",
    "error_extention" : "CSV, TXT 파일 형식에 맞지않습니다.",
